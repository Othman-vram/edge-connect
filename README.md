...
Or you can use the models that I have trained on the `CelebA` dataset with only one epoch present in the `./checkpoints` directory.
...
## Training with MesoNET

Setup (on first connexion to MesoNET):

- ```git clone https://github.com/Othman-vram/edge-connect.git```
- ```cd edge-connect```
- ```spack load py-cython@0.29.33```
- ```spack load py-setuptools@67.6.0```
- ```spack load py-wheel@0.37.1```
- ```spack load python@3.11.9```
- ```python3.11 -m venv .venv```
- ```source .venv/bin/activate```
- ```pip install --upgrade pip```
- ```pip install gdown```
- ```gdown https://drive.google.com/file/d/1h4ShlsyeMaRMlQzpF78HGvCNlwe8iECn/view --fuzzy```
- ```unzip data.zip```
- ```pip install -r requirements.txt```
- ```mkdir datasets```
- ```python3 ./scripts/flist.py --path ./data/celeba/train --output ./datasets/places2_train.flist```
- ```python3 ./scripts/flist.py --path ./data/celeba/test --output ./datasets/places2_test.flist```
- ```python3 ./scripts/flist.py --path ./data/celeba/val --output ./datasets/places2_val.flist```
- ```python3 ./scripts/flist.py --path ./data/masks/train --output ./datasets/masks_train.flist```
- ```python3 ./scripts/flist.py --path ./data/masks/test --output ./datasets/masks_test.flist```
- ```python3 ./scripts/flist.py --path ./data/masks/val --output ./datasets/masks_val.flist```
- Fix the PyTorch error mentioned above

For each connexion:
- ```source .venv/bin/activate```

Starting a training:
- Edge Generation : ```sbatch ./scripts/train_edge_generation_slurm.sh``` (Parametered for : 1 H, 1 GPU)

Check job status:
- Your jobs :
	- ```squeue -u your_username```
- All jobs :
	- ```squeue --partition=mesonet```
- PD = Pending
- R = Running
...
Output:
- Inside slurm-xxxxx.out with xxxxx the job ID
